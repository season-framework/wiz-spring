...
client
  │
  ▼
HTTP validation ── concurrency gate ── wiz-spring 1.2.2 JAR
                                            │
                                            ▼
...
`target/wiz-spring-1.2.2.jar` read-only at `/opt/wiz-source/wiz-spring.jar`. At
...
| `WIZ_SPRING_JAR` | `../target/wiz-spring-1.2.2.jar` | Generator path used by the Go process |
...
```bash
sha256sum target/wiz-spring-1.2.2.jar
...
Node.js/npm, and `target/wiz-spring-1.2.2.jar`:
