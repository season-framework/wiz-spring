...
client
  │
  ▼
HTTP validation ── concurrency gate ── wiz-spring 1.2.0 JAR
                                            │
                                            ▼
...
`target/wiz-spring-1.2.0.jar` read-only at `/opt/wiz-source/wiz-spring.jar`. At
...
Edit the checked-in [`helper/.env`](../.env), which already contains runnable defaults,
and pass it explicitly. There is no example-file copy step:
...
| `WIZ_SPRING_JAR` | `../target/wiz-spring-1.2.0.jar` | Generator path used by the Go process |
...
```bash
sha256sum target/wiz-spring-1.2.0.jar
...
Node.js/npm, and `target/wiz-spring-1.2.0.jar`:
