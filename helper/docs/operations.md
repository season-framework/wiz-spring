...
client
  │
  ▼
HTTP validation ── concurrency gate ── wiz-spring 1.1.1 JAR
                                            │
                                            ▼
...
`target/wiz-spring-1.1.1.jar` read-only at `/opt/wiz-source/wiz-spring.jar`. At
...
| `WIZ_SPRING_JAR` | `../target/wiz-spring-1.1.1.jar` | Generator path used by the Go process |
...
Node.js/npm, and `target/wiz-spring-1.1.1.jar`:
