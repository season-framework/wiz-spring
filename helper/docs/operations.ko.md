...
client
  │
  ▼
HTTP 검증 ── 동시성 gate ── wiz-spring 1.0.0 JAR
                                      │
                                      ▼
                               base 프로젝트 생성
                                      │
...
`target/wiz-spring-1.0.0.jar`를 읽기 전용으로
...
| `WIZ_SPRING_JAR` | `../target/wiz-spring-1.0.0.jar` | Go process가 사용하는 generator 경로 |
...
3. `java -jar ... --version` 결과가 정확히 `wiz-spring 1.0.0`인지 확인합니다.
