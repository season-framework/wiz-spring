...
client
  │
  ▼
HTTP 검증 ── 동시성 gate ── wiz-spring 1.2.1 JAR
                                      │
                                      ▼
                               base 프로젝트 생성
                                      │
...
`target/wiz-spring-1.2.1.jar`를 읽기 전용으로
...
기본값이 포함된 [`helper/.env`](../.env)를 직접 수정한 뒤 명시적으로 전달합니다.
별도 example 파일을 복사할 필요가 없습니다.
...
| `WIZ_SPRING_JAR` | `../target/wiz-spring-1.2.1.jar` | Go process가 사용하는 generator 경로 |
...
3. `java -jar ... --version` 결과가 정확히 `wiz-spring 1.2.1`인지 확인합니다.
...
`target/wiz-spring-1.2.1.jar`가 필요합니다.
