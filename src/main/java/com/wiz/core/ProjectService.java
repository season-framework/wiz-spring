...
import com.wiz.migration.ProjectInventoryService;
...
public class ProjectService {
    ...
    public ProjectService(PathService paths) {
        this(paths, new ProjectInventoryService());
    }

    @Autowired
    public ProjectService(PathService paths, ProjectInventoryService inventory) {
        this.paths = paths;
    ...
    public ProjectContext createProject(String name, String uri, Path sourcePath, boolean generateJavaStubs) throws IOException, InterruptedException {
        ...
        inventory.writeReportIfPythonProject(project.root(), generateJavaStubs);
        return project;
    }
    ...
    private Optional<Path> configuredTemplate() {
        return templatePath(System.getProperty("wiz.template.java-sample"))
                .or(() -> templatePath(System.getenv("WIZ_JAVA_SAMPLE_TEMPLATE")));
    }

    private Optional<Path> templatePath(String value) {
        if (value == null || value.isBlank()) {
            return Optional.empty();
        }
        Path path = Path.of(value).toAbsolutePath().normalize();
        return isJavaSampleTemplate(path) ? Optional.of(path) : Optional.empty();
    }

    private Optional<Path> findTemplateFromCodeSource() {
        try {
            Path location = Path.of(ProjectService.class.getProtectionDomain().getCodeSource().getLocation().toURI()).toAbsolutePath().normalize();
            Path start = Files.isRegularFile(location) ? location.getParent() : location;
            return findTemplateFrom(start);
        } catch (IllegalArgumentException | URISyntaxException exception) {
            return Optional.empty();
        }
    }

    private Optional<Path> findTemplateFrom(Path start) {
        Path current = start;
        while (current != null) {
            for (Path candidate : List.of(
                    current.resolve("wiz-sample-project-java"),
                    current.resolve("../wiz-sample-project-java").normalize())) {
                if (isJavaSampleTemplate(candidate)) {
    ...
    private boolean isJavaSampleTemplate(Path path) {
        return Files.isDirectory(path)
                && Files.isRegularFile(path.resolve("src/app/page.access/api.java"))
                && Files.isRegularFile(path.resolve("src/app/page.dashboard/api.java"))
                && Files.isRegularFile(path.resolve("src/model/Struct.java"))
                && Files.isRegularFile(path.resolve("src/portal/post/app/list/api.java"));
    }
    ...
    private void rewriteTemplateProjectPackage(ProjectContext project) throws IOException {
        ...
        try (Stream<Path> paths = Files.walk(project.root())) {
            for (Path file : paths.filter(path -> Files.isRegularFile(path) && path.toString().endsWith(".java")).toList()) {
