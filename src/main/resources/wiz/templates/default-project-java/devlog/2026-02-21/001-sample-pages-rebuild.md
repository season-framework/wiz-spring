...
### App (신규 생성)
- `src/app/page.access/` — 로그인 (app.json, view.ts, view.pug, api.java)
- `src/app/page.dashboard/` — 대시보드 (app.json, view.ts, view.pug, api.java)
- `src/app/page.posts/` — 게시물 목록 (app.json, view.ts, view.pug, api.java)
- `src/app/page.posts.item/` — 게시물 상세 (app.json, view.ts, view.pug)
- `src/app/page.members/` — 멤버 관리 (app.json, view.ts, view.pug, api.java)
- `src/app/page.mypage/` — 내 프로필 (app.json, view.ts, view.pug, api.java)

### Component (수정)
- `src/app/component.nav.sidebar/view.pug` — 로고를 아이콘+텍스트로 변경, 메뉴 항목을 Dashboard/게시물/멤버로 교체

### Controller (수정)
- `src/controller/UserController.java` — portal/infra/struct 참조 제거, TODO 주석으로 대체
